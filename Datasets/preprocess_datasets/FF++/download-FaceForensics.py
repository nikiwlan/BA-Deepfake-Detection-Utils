...
def parse_args():
    ...
    parser.add_argument('-n', '--num_videos', type=int, default=None,
                        help='Select a number of videos to '
...
def main(args):
    ...
    for dataset in c_datasets:
        ...
        # Get file lists and video lengths list from server
        if 'DeepFakeDetection' in dataset_path or 'actors' in dataset_path:
            ...
            filelist = [file for pair in file_pairs for file in pair]
        else:
            # Load file list from server
            file_pairs = json.loads(urllib.request.urlopen(args.base_url + '/' +
                FILELIST_URL).read().decode("utf-8"))
            filelist = ['_'.join(pair) for pair in file_pairs]
            if c_type != 'models':
                filelist += ['_'.join(pair[::-1]) for pair in file_pairs]
        ...
        dataset_videos_url = f'{args.base_url}{dataset_path}/{c_compression}/{c_type}/'
        ...
        # Else: models for Deepfakes
            ...
            for folder in tqdm(filelist):
                ...
                folder_base_url = f'{args.deepfakes_model_url}{folder}/'
