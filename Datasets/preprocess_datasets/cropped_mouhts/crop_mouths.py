...
DATASETS = {
    ...
    "DFDC": ["DFDC"],
    "CustomDataset": ["CustomDataset/Real", "CustomDataset/Fake"],
}
STD_SIZE = (256, 256)
...
def crop_video_and_save(video_path, landmarks_dir, target_dir, mean_face_landmarks, args):
    ...
    for i, frame_name in enumerate(frame_names):
        ...
        common_path = os.path.join(landmarks_dir, f"{frame_name[:-4]}.npy")
        if os.path.exists(common_path):
            landmarks = np.load(common_path)
        else:
            print(f"Landmarks not found for frame {frame_name}. Skipping frame.")
            continue  # Skip the frame if any landmark is missing
